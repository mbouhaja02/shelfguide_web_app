...
export function AppHeader({ onMenuToggle }: AppHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b border-border/50 glass px-4 lg:px-6">
      <button className="lg:hidden p-2 -ml-2 rounded-lg hover:bg-black/5 transition-colors" onClick={onMenuToggle}>
      ...
      <div className="flex items-center gap-1.5 ml-auto">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="flex items-center gap-2.5 py-1 px-2 rounded-lg hover:bg-black/5 transition-colors cursor-pointer outline-none">
            <Avatar className="h-7 w-7 ring-2 ring-primary/10">
              <AvatarFallback className="bg-gradient-to-br from-indigo-500 to-violet-600 text-white text-[10px] font-bold">{initials}</AvatarFallback>
            </Avatar>
